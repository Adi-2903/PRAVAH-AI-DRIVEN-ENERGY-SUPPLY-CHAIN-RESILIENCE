...
> **Scope note (post-merge):** this gate audits the **`api.py` monolith + Vercel/Render**
> path. The repo also contains the per-agent **microservices** path (Docker + `shared/auth.py`
> + `coordinator/`) merged from Preetansh, which supplies its own Docker/auth/DB and tests.
> "Docker N/A" and "Auth N/A" below refer to the **monolith path only**.
