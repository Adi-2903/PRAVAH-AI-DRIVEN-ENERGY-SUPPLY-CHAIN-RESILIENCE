...
This guide covers deploying the **`api.py` monolith** — **Option A (Vercel only) is recommended:
one platform, free, no second host, no CORS.** (The repo also ships the per-agent
**microservices** stack — `docker-compose.yml` + `run_servers.ps1` + `coordinator/` +
`shared/auth.py` — as an alternative for local multi-service development.)
