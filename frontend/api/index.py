...
_inner.add_middleware(CORSMiddleware, allow_origins=_origins, allow_methods=["*"], allow_headers=["*"])
log.info("CORS allow_origins = %s", _origins)


@_inner.middleware("http")
...
# ── scenario routes ─────────────────────────────────────────────────────────
@_inner.get("/data-status")
def data_status():
    return get_data_status()


@_inner.get("/simulate/mock", response_model=SimulateResponse)
def simulate_mock():
    return _simulate_mock()


@_inner.post("/simulate", response_model=SimulateResponse)
def simulate(req: SimulateRequest):
    return _simulate(req)


# ── spr routes ──────────────────────────────────────────────────────────────
@_inner.post("/spr-schedule", response_model=SPRScheduleResponse)
def spr_schedule(req: SPRScheduleRequest):
    return _spr_schedule(req)


@_inner.get("/spr-schedule/mock", response_model=SPRScheduleResponse)
...
@_inner.post("/recommend", response_model=RecommendResponse)
async def recommend(req: RecommendRequest):
    return await _recommend(req)


# ── risk routes ─────────────────────────────────────────────────────────────
@_inner.post("/risk-score", response_model=RiskScoreResponse)
def risk_score(req: RiskScoreRequest, live: bool = False):
    return build_risk_response(req.corridor, req.as_of, use_live=live)


@_inner.get("/corridors", response_model=List[RiskScoreResponse])
