...
> **Scope note (post-merge):** this report assesses the **`api.py` monolith + Vercel/Render**
> deployment path (the recommended, simplest one). The repository ALSO contains the
> per-agent **microservices** path (Docker + `shared/auth.py` + `coordinator/`), merged
> from the Preetansh line — those bring their own Docker/auth/DB pieces and are run/tested
> separately (`docker-compose.yml`, `run_servers.ps1`, `tests/`). So the "Docker removed /
> auth N/A" determinations below apply to the **monolith path only**, not the whole repo.
